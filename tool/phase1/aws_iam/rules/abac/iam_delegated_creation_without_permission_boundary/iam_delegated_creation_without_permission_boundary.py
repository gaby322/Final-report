...
from phase1.aws_iam.rules.abac.abac_rule_utils import (
    ...
    statement_has_permission_boundary_condition,
    statement_allows_any_action_from_set,
)

_DELEGATED_CREATE_ACTIONS: frozenset = frozenset({
    "iam:createuser",
    "iam:createrole",
})


class iam_delegated_creation_without_permission_boundary(ABACCheck):
    ...
    def execute(self) -> Optional[Dict[str, Any]]:
            ...
            for principal_name, principal in principal_iter:
                ...
                for policy in inline + attached:
                    ...
                    for idx, stmt in enumerate(policy_statements(doc)):
                        if not statement_allows_any_action_from_set(stmt, _DELEGATED_CREATE_ACTIONS):
                            continue
                        if not statement_has_wildcard_resource(stmt):
                            continue
                        if statement_has_permission_boundary_condition(stmt):
                            continue
                        violations.append({
                            ...
                            "reason": (
                                "iam:CreateUser or iam:CreateRole on Resource:* without "
                                "iam:PermissionsBoundary condition — enables privilege escalation "
                                "via unconstrained delegated entity creation"
                            ),
                        })
        ...
        if not violations:
            return {
                "status": "PASS",
                "status_extended": (
                    "All policies granting iam:CreateUser or iam:CreateRole on Resource:* "
                    "include iam:PermissionsBoundary conditions."
                ),
                "resource_type": "AWS::IAM::Policy",
                "resource_id": self.provider.identity.account_id,
                "category": "IAM",
                "evidence_summary": "All delegated IAM creation grants enforce permission boundaries",
                "evidence_details": {
                    "violations": [],
        ...
        return {
            "status": "FAIL",
            "status_extended": (
                f"{len(violations)} policy statement(s) grant iam:CreateUser or "
                "iam:CreateRole on Resource:* without iam:PermissionsBoundary conditions, "
                "enabling privilege escalation via unconstrained delegated entity creation."
            ),
            "resource_type": "AWS::IAM::Policy",
            "resource_id": self.provider.identity.account_id,
            "category": "IAM",
            "evidence_summary": (
                f"{len(violations)} delegated IAM creation grant(s) without permission "
                "boundary enforcement — direct privilege escalation vector"
            ),
            "evidence_details": {
