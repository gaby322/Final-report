...
class iam_inline_policy_no_full_access_to_kms(RBACCheck):
    def execute(self) -> Optional[Dict[str, Any]]:
        violations: List[Dict[str, Any]] = []

        def _scan(principal: Any, rtype: str, pname: str) -> None:
            for policy in getattr(principal, "inline_policies", []):
                if policy_document_has_full_kms(getattr(policy, "document", {}) or {}):
                    violations.append({
                        "resource_type": rtype,
        ...
        if violations:
            return {
                "status": "FAIL",
                "status_extended": "Inline policies grant full KMS access",
                "resource_type": "AWS::IAM::Policy",
                "resource_id": self.provider.identity.account_id,
                "category": "IAM",
                "evidence_summary": f"{len(violations)} inline policies grant full KMS access",
                "evidence_details": {"violations": violations},
            }

        return {
            "status": "PASS",
            "status_extended": "No inline policies grant full KMS access",
            "resource_type": "AWS::IAM::Policy",
            "resource_id": self.provider.identity.account_id,
            "category": "IAM",
            "evidence_summary": "Inline policy documents do not grant full KMS access",
